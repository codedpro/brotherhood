...
>(({ className, children }, ref) => {
  return (
    <div
      ref={ref}
      className={cn(
        "flex items-center justify-center w-12 h-12 rounded-full border-2 border-border bg-white shadow-[0_0_20px_-12px_rgba(0,0,0,0.8)]",
        className
      )}
    >
      {children}
...
export function Mining({ className }: { className?: string }) {
  const totalIcons = 10;
  const radius = 150;

  const centralIconRef = useRef<HTMLDivElement>(null);
  const iconRefs = useRef<Array<React.RefObject<HTMLDivElement>>>([]);
  const containerRef = useRef<HTMLDivElement>(null);

  if (iconRefs.current.length !== totalIcons) {
    iconRefs.current = Array.from({ length: totalIcons }, () =>
      React.createRef<HTMLDivElement>()
    );
  }

  const areAllRefsSet =
    centralIconRef.current && iconRefs.current.every((ref) => ref.current);

  return (
    <div
      className={cn(
        "relative flex items-center justify-center h-[500px] w-full mx-auto",
        className
      )}
      ref={containerRef}
    >
      ...
      <div className="relative w-full h-full">
        {iconRefs.current.map((iconRef, index) => {
